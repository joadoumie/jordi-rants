...
![Maeby](https://github.com/joadoumie/jordi-rants/blob/5476027a2c981ba32efa555328efbf4386a4815c/assets/maeby.jpeg)
...
But there's a newer and underrated pattern: **MCP Apps**. This is the thing I was really interested in playing with. Instead of just exposing raw tools, an MCP App ships an interactive UI that renders right inside the AI client. Conversational agents can invoke a tool that returns a full HTML interface, and the user interacts with it without ever leaving the conversation. So you can build an actualy visual interface that folks can access from a bunch of different compatible surfaces. Seemed pretty neat to me.
...
Check out the full source on [GitHub]([GITHUB_REPO_PLACEHOLDER]) if you want to poke around, contribute, or riff on the idea.
